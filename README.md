...
- Install Dependence
    ...
    ```
- Validate Dependence in powershell
    ```
    pip list | Select-String 'flask|requests' 
    ```
- Run locally. 
    ```
    python app.py
...
We will create Dockerfile. If you don't know what is docker, please read some basic understanding about docker: https://www.docker.com/
...
```
docker build -t gchauhan1517/flask-app:v1 .
```
...
1. Create a directory inside your project.
    ...
    build:
        ...
        - name: Build Docker Image
            run: |
            docker build . -t gchauhan1517/demo-app:${{ env.VERSION }} 
        
        # Push the Docker Image
        - name: Push Docker Image
            run: |
            docker push gchauhan1517/flask-app:${{ env.VERSION }}
        
        # UPdate the K8s Manifest Files
        - name: Update K8s Manifests
            run: |
            cat deploy/deploy.yaml
            sed -i "s|image: gchauhan1517/flask-app:.*|image: gchauhan1517/flask-app:${{ env.VERSION }}|g" deploy/deploy.yaml
            cat deploy/deploy.yaml
